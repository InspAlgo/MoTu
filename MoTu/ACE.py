...
def zmIce(I, ratio=4, radius=300):  # 常规的ACE实现
    ...
    zh, zw = [0]*radius + list(range(height)) + [height-1] * \
        radius, [0]*radius + list(range(width)) + [width - 1]*radius
    Z = I[np.ix_(zh, zw)]
    ...
    for h in range(radius*2+1):
        for w in range(radius*2+1):
            if para[h][w] == 0:
                continue
            res += (para[h][w] *
                    np.clip((I-Z[h:h+height, w:w+width])*ratio, -1, 1))
    return res
...
def FACE():
    m = zmIceColor(cv2.imread('template.png')/255.0)*255
    cv2.imwrite('enhance.jpg', m)
